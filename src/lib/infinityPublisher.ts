...
export async function publishToInfinitySpark(page: BuildPage): Promise<PublishResult> {
  try {
    ...
    const userFolder = user.login.toLowerCase()
    const pageUrl = `https://c13b0.github.io/infinity-spark/${userFolder}/${slug}/`
    ...
    await spark.kv.set(`infinity-published-${page.id}`, pageData)
    
    const registry = await spark.kv.get<any[]>('infinity-page-registry') || []
    ...
    await spark.kv.set('infinity-page-registry', registry)

    return {
      success: true,
      url: pageUrl,
      message: `Page will be published to infinity-spark repository. The owner will deploy it shortly.`
    }
  } catch (error) {
    console.error('[publishToInfinitySpark error]', error)
    return {
      success: false,
...
export async function getPublishedPageData(pageId: string) {
  return await spark.kv.get(`infinity-published-${pageId}`)
}
...
export async function downloadPageForDeploy(pageId: string) {
  const pageData = await spark.kv.get<any>(`infinity-published-${pageId}`)
